...
RSpec.describe Pirate do
  ...
  it 'has a booty' do
        pirate = Pirate.new('Pete')
        expect(pirate.booty).to eq(0)

  end

  it 'gets 100 booty for robbing a ship' do
      pirate = Pirate.new('Sue')

      expect(pirate.robs_ships).to eq(100)

      expect(pirate.robs_ships).to eq(200)

      expect(pirate.robs_ships).to eq(300)

      expect(pirate.robs_ships).to eq(400)
  end
